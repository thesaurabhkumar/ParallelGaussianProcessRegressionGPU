...
// Kernel code to run the cholesky factorization routine
__global__ void gpr_cholesky(int N, double *K, double *L, double *partial_sum) {
    // LU factorization
    for(int k = 0; k < n; k++) {
        for(int i = 0; i < n; i++) {
            if(i == k) {
...
__global__ void gpr_solver(int N, double *Y, double *z, double *L, double *partial_sum, double *f)
{
    ...
    for(int i = n-1; i >= 0; i--) {
        ...
        for(int j = n-1-threadIdx.x; j > i; j -= N) {
            partial_sum[threadIdx.x] += (L[j * n + i] * z[j]); // U component is nothing but L'
        }
        __syncthreads();
...
__global__ void gpr_predict(int N, int m, double a, double b, double *k, double *z, double *partial_sum, struct XY *xy)
{
...
int main(int argc, char* argv[]) {
    ...
    double *f, *k, *Y, *z, *K, *L, *partial_sum;
    struct XY *xy;
    ...
    gpr_cholesky<<<1,num_threads>>>(num_threads, K, L, partial_sum);
    cudaEventRecord(stop_cholesky, 0);
    cudaEventSynchronize(stop_cholesky);
    cudaEventRecord(start_solver, 0);
    gpr_solver<<<1,num_threads>>>(num_threads, Y, z, L, partial_sum, f);
    cudaEventRecord(stop_solver, 0);
    cudaEventSynchronize(stop_solver);
    gpr_predict<<<1,num_threads>>>(num_threads, m, rstar[0], rstar[1], k, z, partial_sum, xy);
...
//    printf("Floating point operations per second (FLOPS): %f Gflops\n", (-1)/(elapsedTime_kernel/1000.0)/(1024.0*1024*1024)); //Update Needed

    double *y = (double *)malloc(sizeof(m*m*sizeof(double)));
    cudaMemcpy(y, f, m*m*sizeof(double), cudaMemcpyDeviceToHost);
    for(int i = 0; i < m*m; i++)
        printf("%f \n", y[i]);
